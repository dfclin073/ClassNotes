...
function sudo	{start-process -filepath $args[0] -verb runas}
...
function ip {start-process -verb runas ncpa.cpl}
function nc {Test-NetConnection -InformationLevel Detailed -Computername $args[0] -port $args[1]}

function Relaunch-Admin { Start-Process -Verb RunAs cmd.exe '/c start wt.exe'}
Set-Alias su Relaunch-Admin